...
import { createStudent, deleteStudent, getRole, getStudents, hasSession, login, register, updateStudent, clearTokens } from './api'
import './App.css'
...
function App() {
  ...
  async function submitLogin(event) {
    ...
    setLoading(true)
    setError('')
    try { const tokens = await login(credentials); setRole(tokens.role); setAuthenticated(true) } catch (requestError) { setError(requestError.message) } finally { setLoading(false) }
  }
  ...
  async function submitStudent(event) {
    ...
    setLoading(true)
    setError('')
    try { const payload = { ...form, semester: Number(form.semester), cgpa: form.cgpa ? Number(form.cgpa) : null }; if (editingId) await updateStudent(editingId, payload); else await createStudent(payload); setShowModal(false); setForm(blankStudent); setEditingId(null); await loadStudents() } catch (requestError) { setError(requestError.message) } finally { setLoading(false) }
  }
  ...
  function editStudent(student) { setForm({ ...blankStudent, ...student, cgpa: student.cgpa ?? '' }); setEditingId(student.id); setShowModal(true) }

  const activeCount = useMemo(() => students.length, [students])
  if (!authenticated) return <Login credentials={credentials} setCredentials={setCredentials} submitLogin={submitLogin} loading={loading} error={error} onAuthSuccess={(tokens) => { setRole(tokens.role); setAuthenticated(true) }} />

  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">N</span><span>Northstar</span></div><p className="eyebrow">Workspace</p><nav><button className="nav-item active"><span>▦</span> Students</button><button className="nav-item" onClick={() => setError('Courses management is not available yet.') }><span>▤</span> Courses</button><button className="nav-item" onClick={() => setError('Attendance management is not available yet.') }><span>◷</span> Attendance</button><button className="nav-item" onClick={() => setError('Reports are not available yet.') }><span>▥</span> Reports</button></nav><div className="sidebar-bottom"><button className="nav-item" onClick={() => setError('Settings are not available yet.')}><span>⚙</span> Settings</button><div className="profile"><div className="avatar">{role.slice(0, 2)}</div><div><strong>{role || 'User'}</strong><small>Authenticated</small></div><button className="profile-menu" onClick={() => { clearTokens(); setRole(''); setAuthenticated(false) }}>↪</button></div></div></aside>
    <main className="main-content"><header className="topbar"><div className="crumb">Workspace <span>/</span> <strong>Students</strong></div><div className="top-actions"><span className="connection-status"><i /> API connected</span></div></header><section className="page-heading"><div><p className="eyebrow">Student management</p><h1>Student directory <span>✦</span></h1><p className="subheading">Create, search, and maintain student records.</p></div>{canWrite && <button className="primary-button" onClick={() => { setForm(blankStudent); setEditingId(null); setShowModal(true) }}><span>+</span> Add student</button>}</section>
...
function Login({ credentials, setCredentials, submitLogin, loading, error, onAuthSuccess }) {
  ...
  async function submit(event) {
    ...
    try { setFormError(''); const tokens = await register(details); onAuthSuccess(tokens) } catch (requestError) { setFormError(requestError.message) }
  }
